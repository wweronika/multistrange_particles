...
def plot(scaleCheck = "n"):
    ...
    if scaleCheck == "y":        
        FactorToOne(MCXiData[:,num], alpha = 0.8, label = "MC Omega")
        FactorToOne(RealXiData[:,num], label = "Real Omega")
        yText = 'Scalar Factored Counts (Arbitary Units)'
 
    else:
        plt.hist(MCXiData[:,num], bins = standardBins, alpha=0.8, label='MC Omega')
        plt.hist(RealXiData[:,num], bins = standardBins, alpha=0.5, label='Real Omega')
        yText = "Counts"
...
def noGraphFit(RealXiDataClone, bounds = ([30,1.31,0,-5,-30,0,0], [150,1.34,0.01,10,5,200,500]), Bins = 500):
    (nHits, bins, patches) = plt.hist(RealXiDataClone[:,1], bins=Bins, label= 'Real Omega', range = [1.28,1.55])
    plt.clf()
    x = bins[:-1]
    y = nHits
...
def cuts():
    ...
    while True:
        ...
        cutType = input("Up and Down (y) or Inbetween (n)?")
        ...
        if cutType == "y":
            ...
            x = 0
            while x < len(RealXiData):
                if RealXiData[x,num] > lowerLim and RealXiData[x,num] < UpperLim:
                    RealXiData = np.delete(RealXiData, x, 0)
                else:
                    x += 1
