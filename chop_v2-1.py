...
def significance(peak,background):
    return peak/np.sqrt(background)
...
def gPlusPolyFit():
    ...
    (nHits, bins, patches) = plt.hist(RealXiDataClone[:,1], bins=500, label= 'Real Xi', range = [1.28,1.55])
    ...
    popt,pcov = curve_fit(GplusPoly, x, y, bounds=([1,1.31,0,-5,-30,0,0], [150,1.34,0.01,10,5,200,500]))
...
while vChoice != "0":
   ...
   # choice  6
        ...
        backgroundIntregral = np.zeros(2)
        
        column = 1
        for i in np.linspace(np.min(RealXiData[:,column]),np.max(RealXiData[:,column]),num=100):
            RealXiDataClone = RealXiData
            x = 0
            while x < len(RealXiDataClone):
                if RealXiDataClone[x,1] < i:
                    RealXiDataClone = np.delete(RealXiDataClone, x, 0)
                else:
                    x += 1
                    
            (fit) = gPlusPolyFit()
            signalIntegral = np.vstack((signalIntegral, integrator(fit, standToValue(-2,fit), standToValue(2,fit))))                
            backgroundIntregral = np.vstack((backgroundIntregral,integrator(fit, standToValue(-7,fit), standToValue(-5,fit))))
            
        signalIntegral = np.delete(signalIntegral, 0, 0)
        backgroundIntregral = np.delete(backgroundIntregral, 0, 0)                       
