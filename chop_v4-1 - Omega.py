...
########################################
def noGraphFit(RealXiDataClone, bounds = ([5,1.66,0,-5,-30,0,0], [30,1.68,0.01,10,5,200,500]), Bins = 50, Range = [1.6,1.825]):
...
def gPlusPolyFit(data, Bounds = ([5,1.66,0,-5,-30,0,0], [30,1.68,0.01,10,5,200,500]), Column = 1, Bins = 50, Range = [1.6,1.75]):
...
def significance_calculations():
    ...
    for i in cutPoints:
        ...
        try:    #Check if Guassian can still be fitted on data set
            (fit) = gPlusPolyFit(RealXiDataClone, Bins = 50)
        except RuntimeError:
            print(j)
...
while vChoice != "0":
    ...
    # choice  4
        ...
        (fit) = gPlusPolyFit(RealXiData, Range = [1.62,1.75]) #[min(RealXiData[:,1]),1.75])
        print("Mean: ",fit[1])
