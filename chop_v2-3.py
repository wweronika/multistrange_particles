...
while vChoice != "0":
   ...
   # choice  6
        ...
        cutPoints = np.linspace(np.min(RealXiData[:,column]),np.max(RealXiData[:,column]),num=100)   
        j = 1
        ...
        if (abs(Max - largestBin) < 10*Binwidth) or (abs(Min - largestBin) < 10*Binwidth):
            ...
            (fit) = gPlusPolyFit(RealXiData, Bounds = ([1,1.115,0,-5,-30,0,0], [150,1.1175,0.001,10,5,200,500]), Column = column, Bins = 200, Range = None)
            lowerSigma = fit[1] - 2*fit[2]    
            upperSigma = fit[1] + 2*fit[2]
            
            lowerPoints = np.linspace(np.min(RealXiData[:,column]),lowerSigma,num=50)
            HigherPoints = np.flip(np.linspace(upperSigma,np.max(RealXiData[:,column]),num=50))
        ...
        if vType != "Guass":
            ...
            plt.plot(cutPoints[:49], signalIntegral[:49,0],label='Signal',color='r',linestyle='--')
            plt.plot(cutPoints[:49], backgroundIntregral[:49,0],label='Background',color='b',linestyle='--')
            plt.plot(cutPoints[:49], signif[:49],label='Significance',color='g')
            
            plt.plot(cutPoints[50:], signalIntegral[50:,0],color='r',linestyle='--')
            plt.plot(cutPoints[50:], backgroundIntregral[50:,0],color='b',linestyle='--')
            plt.plot(cutPoints[50:], signif[50:],color='g')
        
        plt.xlabel("Cut value of:  "+xAxis[5])
